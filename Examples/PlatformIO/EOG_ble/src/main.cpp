...
unsigned long interval = 4; // Interval between samples in milliseconds (1000 ms / 500 samples). Change as per requirement
...
void loop(){ 
  ...
  if (currentMillis - previousMillis >= interval) {
    ...
    float filters_out = exg_synapse.apply_EOG_filters(sensorValue); // generate output after applying filters
